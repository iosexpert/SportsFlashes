...
@interface liveScoreCategoryView ()
{
    NSMutableArray *listArr;
}
@end

@implementation liveScoreCategoryView

- (void)viewDidLoad {
    [super viewDidLoad];
    catTableView.backgroundColor=[UIColor whiteColor];
...
-(void)viewWillAppear:(BOOL)animated
{
    if([Utilities CheckInternetConnection])//0: internet working
    {
        ...
        NSMutableURLRequest *request = [httpClient requestWithMethod:@"GET"
                                                                path:[NSString stringWithFormat:@"/api/v3/subcat?ln=%@",lang]
                                                          parameters:nil];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
     ...
     UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"CELL"];
        ...
        CGRect labelFrame = CGRectMake (12, 3, 180, 30);
    ...
    CGRect labelFrame1 = CGRectMake (0, 0, 30, 20);
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    {
        footbalMatchListView *smvc;
        int height = [UIScreen mainScreen].bounds.size.height;
        if (height == 480) {
            
            
            smvc =[[UIStoryboard storyboardWithName:@"iphone4" bundle:nil] instantiateViewControllerWithIdentifier:@"footbalMatchListView"];
            
        }
        else if (height == 568) {
            
            
            smvc =[[UIStoryboard storyboardWithName:@"Main" bundle:nil] instantiateViewControllerWithIdentifier:@"footbalMatchListView"];
            
        }
        else if (height == 667) {
            
            
            smvc =[[UIStoryboard storyboardWithName:@"iphone6" bundle:nil] instantiateViewControllerWithIdentifier:@"footbalMatchListView"];
            
        }
        else if (height == 736) {
            
            
            smvc =[[UIStoryboard storyboardWithName:@"iphone6plus" bundle:nil] instantiateViewControllerWithIdentifier:@"footbalMatchListView"];
            
        }
        else if (height == 1024) {
            
            
            smvc =[[UIStoryboard storyboardWithName:@"ipad" bundle:nil] instantiateViewControllerWithIdentifier:@"footbalMatchListView"];
        ...
        else
        {
            ...
            smvc =[[UIStoryboard storyboardWithName:@"ipad2" bundle:nil] instantiateViewControllerWithIdentifier:@"footbalMatchListView"];
